...
public class MainActivity extends AppCompatActivity {
    ...
    public void displayDetails(String title, String subTitle, int imageResourcesId) {

        View fragmentView = findViewById(R.id.fragment_second);

        if (fragmentView != null) {//если в landscape то просто перезаписывет нажатое
            fragmentManager = getSupportFragmentManager();
            transaction = fragmentManager.beginTransaction();
            transaction.replace(R.id.fragment_second, TextFragment.newInstance(title, subTitle, imageResourcesId));
            transaction.commit();
        } else {//иначе все как в режиме layout
            Intent intent = new Intent(this, DetailsActivity.class);
            intent.putExtra(KEY, title);
            intent.putExtra(KEY2, subTitle);
            intent.putExtra(KEY3, imageResourcesId);
